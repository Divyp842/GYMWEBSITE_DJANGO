...
def signup(request):
    if request.method=="POST":
        ...
        try:
            if User.objects.get(email=email): # type: ignore
                messages.warning(request,"Email is Taken") # type: ignore
                return redirect('/signup')
...
def enroll(request):
    ...
    if request.method=="POST":
          ...
          if not full_name:
            return HttpResponse("Error: Fullname is required", status=400)
          email=request.POST.get('email')
          ...
          query=Enrollment(Fullname=full_name,Email=email,Gender=gender,Phone=phoneNumber,DOB=dob,SelectMembershipplan=member,SelectTrainer=trainer,Reference=reference,Address=address)
          query.save()
